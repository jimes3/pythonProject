...
def fun(X,www):  # 目标函数和约束条件
    ...
    for i in range(1,6):
        data = np.array([row[int(max(0, x[i]*shuti-4)) : int(min(len(df[0]), x[i]*shuti+4+1))]
                         for row in df[int(max(0, x[0]-4)) : int(min(len(df), x[0]+4+1))]]) # 前x后y
        w,w1,w2 = yuansu(data)
        ww .append(w)
...
'''
[2632.200918593065, 7441.643831213292]
[100.92049487109169, 3.1753085893049136]
[5269.851107967506, 5397.72712103266]
'''