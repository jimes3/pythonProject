...
class GaussianMixtureModel:
    ...
    Description: generate gaussian distribution with given mu, sigma_x and x
    Input:  mu          dataType: ndarray   description: features
    Input:  sigma_x       dataType: ndarray   description: features
    ...
    def save(self, filename):
        f = open(filename, 'w')
        model = {'parameters': self.alpha, 'mu': self.mu, 'sigma_x': self.sigma}
        pickle.dump(model, f)
        f.close()
    ...
    def load(self, filename):
        f = open(filename)
        model = pickle.load(f)
        self.alpha = model['parameters']
        self.mu = model['mu']
        self.sigma = model['sigma_x']
        return self