...
#print(rant)
##############################################  平均供货量
x_df = np.array(pd.read_excel("附件1 近5年402家供应商的相关数据.xlsx",sheet_name = '供应商的供货量（m³）').drop(['供应商ID','材料分类'], axis=1))
av_up = []
for i in range(402):
...
s = np.zeros((1,402))
sub = np.array(s+1).ravel()  # 自变量下限
up = np.array(s+1.5).ravel()  # 自变量上限
...
xi = np.array(df['材料分类'])
ress = []
best_x,best_f,trace = woa(sub,up,type,10,40)
print(best_f)
...
for i in range(402):
    res += liang[i]/xi[i][0]
print('产品生产数量：',int(res))
ress.append(int(res))
print('每家供应商供应量：',liang)
print(",".join(str(x) for x in liang))
