...
index = df[(df['发货城市 (Delivering city)'] == 'A') & (df['收货城市 (Receiving city)'] == 'Q')].index.tolist()
#将默认索引改为时间索引
...
model = sm.tsa.arima.ARIMA(train,order=(2,0,1))  #第一个是p，第三个是q
arima_res=model.fit()
arima_res.summary()
predict=arima_res.predict(273,274)   #前闭后闭
print(predict)
