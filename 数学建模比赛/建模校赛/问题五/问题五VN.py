...
index = df[(df['发货城市 (Delivering city)'] == 'V') & (df['收货城市 (Receiving city)'] == 'N')&
           (df['日期(年/月/日) (Date Y/M/D)'] >= '2022-07-01')&(df['日期(年/月/日) (Date Y/M/D)'] <= '2022-09-30')].index.tolist()
df = df.iloc[index,-1]
df = np.array(df)
vn = np.sort(df)
vn = vn[vn>0]
...
for i in range(np.min(vn)+1):
    ...
    popt, pcov = curve_fit(normal_distribution, x, y)
